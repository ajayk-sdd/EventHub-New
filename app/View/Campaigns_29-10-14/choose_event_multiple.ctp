...
<section class="inner-content inner-contentnew">
    <?php echo $this->Form->create("Campaigns", array("action" => "chooseEventMultiple", "name" => "search_forms")); ?>
    <!--Left Section Starts-->
...
if (isset($events) && !empty($events)) {
    ...
    foreach ($events as $event) {
        ...
        ?>
                        ...
                        <a href="javascript:void(0);" class="btn-addEvent" id = '<?php echo $event["Event"]["id"]; ?>' title='<?php echo $event["Event"]["title"]; ?>' onclick='selectThisEvent(this.id, this.title);'>Add</a>                            
        <?php //echo $this->Html->link("Select",array("action"=>"javascript:void(0);"),array("class"=>"btn-addEvent",'id'=>$event["Event"]["id"],"onclick"=>"selectThisEvent(this.id)"));  ?>
                    </li>
        <?php
    }
} else {
